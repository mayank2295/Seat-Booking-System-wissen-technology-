...
const Dashboard = (() => {
  ...
  async function loadSeats() {
    ...
    try {
      ...
      // Check for temp floaters (released seats)
      seats.forEach(s => {
        if (!s.bookedBy && data.seats) {
          const released = (data.stats && data.stats.releasedSeats > 0);
          // Mark released regular seats as temp floaters
        }
      });
    } catch (err) {
  ...
  function renderSeats(seats, user) {
    ...
    seats.forEach((seat) => {
      ...
      if (seat.bookedBy === user.id) {
        ...
        el.title = `Booked by ${seat.bookedBy}`;
  ...
  function updateStats(seats, user) {
    ...
    const floaters = seats.filter(s => s.tempFloater && !s.bookedBy).length;
